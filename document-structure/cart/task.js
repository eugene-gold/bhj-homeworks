...
document.querySelector('.products').addEventListener('click', (event) => {
    ...
    if (event.target.closest('.product__quantity-control_dec')) {
         decr(event)
    } else if (event.target.closest('.product__quantity-control_inc')) {
         incr(event)
    }

    if (event.target.closest('.product__add') && addQuantityButton !== 0) {
        if(Array.from(cart.querySelectorAll('.cart__product')).find(item => item.dataset.id === event.target.closest('.product').dataset.id)) {

            Array.from(cart.querySelectorAll('.cart__product')).find(item => {
                if (item.dataset.id === event.target.closest('.product').dataset.id) {
                    item.querySelector('.cart__product-count').textContent = parseInt(item.querySelector('.cart__product-count').textContent) + parseInt(event.target.closest('.product').querySelector('.product__quantity-value').textContent)
                }
            } )
        } else {
            addProduct(event)
...
const decr = (event) => {
    if (event.target.closest('.product').querySelector('.product__quantity-value').textContent > 1){
